const toGroupStatusCommand = require('./commands/togroupstatus');
...
const { setBotNameCommand, setBotOwnerCommand, setOwnerNumberCommand,setYTChannelCommand,setPackNameCommand,setAuthorCommand, setTimezoneCommand,configHelpCommand } = require('./commands/alsonxmd');
...
const setBotPPCommand = require('./commands/setbotpp'); // 👈 Add this line here
...
// CHATBOT DEBUG
log('🤖 CHATBOT ROUTE DEBUG');
log('├ rawMessageText:', rawMessageText);
log('├ isCommand:', isCommand);
log('├ commandWithoutPrefix:', commandWithoutPrefix);
log('├ chatId:', chatId);
log('├ fromMe:', message.key.fromMe);
log('├ isGroup:', isGroup);
log('└────────────────────────');
...
if (!isCommand) {
    if (rawMessageText.trim()) {
        ...
        log('🤖 BEFORE CHATBOT CALL');
log('🤖 HANDLER TYPE:', typeof handleChatbotResponse);
log('🤖 HANDLER NAME:', handleChatbotResponse?.name);
log('🤖 HANDLER SOURCE:', String(handleChatbotResponse).slice(0, 180));
...
try {
    ...
    log('🤖 AFTER CHATBOT CALL');
} catch (chatbotError) {
    console.error('❌ CHATBOT CALL ERROR:', chatbotError);
}
    }
          ...
          case userMessage === '.botinfo' || userMessage === '.binfo':
                {
                    ...
                    let botInfoText = `🤖 *BOT INFORMATION*\n\n` +
                                    `*Name:* ${settings.botName || 'Alson XMD'}\n` +
...
case userMessage.startsWith('.autorecord'):
    ...
    break;
            ...
            case userMessage.startsWith('.setgdesc'):
                ...
                break;
            case 'setbotpp':
    await setBotPPCommand(sock, chatId, message);
    break;
            ...
            case userMessage.startsWith('.setgname'):
                ...
                break;
            case userMessage.startsWith('.togroupstatus'):
    await toGroupStatusCommand(sock, chatId, message);
    break;
