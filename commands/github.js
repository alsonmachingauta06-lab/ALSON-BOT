...
async function githubCommand(sock, chatId, message) {
    try {
        ...
        const res = await fetch('https://api.github.com/repos/AlsonMachingautah/Alson XMD');
        if (!res.ok) throw new Error(`GitHub API error: ${res.status} ${res.statusText}`);
        
        const json = await res.json();

        // Create professional formatted message
        let txt = `*🤖 Alson XMD REPOSITORY* 🤖\n\n`;
        txt += `📛 *Repository Name:* ${json.name || 'Alson XMD'}\n`;
        ...
        txt += `*© Alson XMD 2025 | All Rights Reserved*`;
